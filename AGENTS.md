...
- **Agent Dependencies**: If the agent requires dependencies not present in the base image, it should suggest adding them to:
  * for opencode - `.images/opencode.Dockerfile` and `.local/bin/opencode.sh`
  * for claude code - `.images/claude.Dockerfile` and `.local/bin/claude.sh`
